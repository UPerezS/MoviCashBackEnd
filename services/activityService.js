const fileName = require("../middlewares/csvMiddleware");
 ...
 */
const addAction = async (RFC, actionData) => {
    try {
        ...
        if (!activity) {
            ...
            // Creacion de el docuimento de actividad para el personal, se crea uno diferente por rol
...
const getFileByName = async(fileName) => {
    try {
        var pipeline = [
            {$unwind: "$Acciones"},
            {$match: {"Acciones.Accion":"Cargar CSV"}},
            {$match:{"Acciones.Detalles":fileName}}
        ];

        const csv = await Actividad.aggregate(pipeline);

        return csv;
    } catch(error) {
        console.error("Error en getFileByName: ", error.message);
        throw new Error("Error al obtener actividades by CSV: "+error.message);
    }
}

module.exports = {addAction, getActivity, getFileByName};